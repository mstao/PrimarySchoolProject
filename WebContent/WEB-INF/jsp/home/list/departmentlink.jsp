				...
				<div class="departmentlink-sp">
					...
					<div class="departmentlink-sp-bottom">
						<c:forEach items="${departmentLinkStaff}"  var="departmentLink_Staff">
							<li><img src="${CTP_HOME}/img/sjiantou.png"/><a href="${CTP}/details/departmentLink?contentId=${departmentLink_Staff.id}&departmentId=${departmentId}&contentFlag=${departmentLink_Staff.typeId}">${myTag:substr(departmentLink_Staff.itemTitle,0,50,true)}   </a><span class="dept-time">[${departmentLink_Staff.addTime}]</span></li>
						</c:forEach>
					</div>
				</div>
			<!-- 部门链接链接中工作安排 -->
				...
				<div class="departmentlink-jp">
					...
					<div class="departmentlink-jp-bottom">
							<c:forEach items="${departmentLinkJob}"  var="departmentLink_Job">
								<li><img src="${CTP_HOME}/img/sjiantou.png"/><a href="${CTP}/details/departmentLink?contentId=${departmentLink_Job.id}&departmentId=${departmentId}&contentFlag=${departmentLink_Job.typeId}">${myTag:substr(departmentLink_Job.itemTitle,0,40,true)}</a><span class="dept-time">[${departmentLink_Job.addTime}]</span></li>
							</c:forEach>
					</div>
				</div>
			</div>
