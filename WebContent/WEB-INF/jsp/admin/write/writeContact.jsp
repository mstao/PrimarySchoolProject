...
<script type="text/javascript">
	...
	<!--S 编辑区域-->
	<div class="edit-div"> 
		<span class="edit-span">撰写联系我们     
		<c:if test="${contact.isPublish eq 0}">
            <b>草稿</b>
            </c:if>    
         </span>
		<span class="content-span">联系我们内容</span><br>
		 <!--S 编辑器 -->              
              <div id="editor-container" class="answercontainer"><div id="editor-trigger">${contact.content}</div></div>
          <!--E 编辑器  -->
          
        <input type="hidden" class="hide-id" value="${contact.id}">
        <div class="edit-submit">
        	<a href="javascript:void(0);" class="draft-btn draft-contact-btn" onClick="ChangeState();">保存草稿</a><a href="javascript:void(0);" class="publish-btn publish-contact-btn" onClick="ChangeState();">发布内容</a>
        </div>
	</div>
	<!--S 编辑区域-->
	...
	<div class="check-zone">
		...
		<div class="options-checked-div" id="s">
		    ...
		    <select name="publish_style" id="publish-style">
		    	<option  selected="true">联系我们</option>
...
$(function(){
  ...
  $('.publish-contact-btn').bind('click',function(){
		...
		if( text_content=="" ||publish_dept==""){
  		...
  		var is_publish=1; //意味着要发表1，不是存为草稿0
			$.ajax({
				type:'post',
				dataType:'text',
				url:CTPPATH+'/admin/dealEditContact',
					...
					setTimeout(function(){
					 ...
					 if(data>0){
						layer.msg('发布成功，您可以在列表中查看', {icon: 1,time:2000});
							//跳转界面
							window.location.href=CTPPATH+"/admin/contact";
  ...
  /**************保存草稿******************/
  $('.draft-contact-btn').bind('click',function(){
