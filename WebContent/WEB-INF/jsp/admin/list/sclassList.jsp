      				...
      				$.ajax({
						...
						success:function(data){
							...
							if(data==1){
								...
								window.location.href="${pageContext.request.contextPath}/admin/list/${durl}?classId=${labId}&grade=${grade}&className=${className}&p=1";
  		...
  		$.ajax({
  			...
  			success:function(data){
  				...
  				$.each(data,function(id,item){
  						if(item.isPublish==0){
  							xhtml+="<tr><td width='5%' align='center'><input type='checkbox' name='info_id' value="+item.id+"></td><td width='35%'><a href='${CTP}/admin/distribute/${durl }?id="+item.id+"&deptId=${deptId}&typeId="+item.typeId+"' class='item_title'>"+item.itemTitle+"</a><span class='draft-span'>草稿</span></td><td width='20%'>"+item.itemAuthor+"</td><td width='20%'>${labName}</td><td width='20%'>"+item.addTime+"</td></tr>";
  							}
  						else{
  							xhtml+="<tr><td width='5%' align='center'><input type='checkbox' name='info_id' value="+item.id+"></td><td width='35%'><a href='${CTP}/admin/distribute/${durl }?id="+item.id+"&deptId=${deptId}&typeId="+item.typeId+"' class='item_title'>"+item.itemTitle+"</a></td><td width='20%'>"+item.itemAuthor+"</td><td width='20%'>${labName}</td><td width='20%'>"+item.addTime+"</td></tr>";
  						}
        ...
        <div class="new_div3">            
            <table>
                ...
                <tr>
                    ...
                    <td width="35%"><a href="${CTP}/admin/distribute/${durl }?id=${list.id}&grade=${grade}&className=${className}&typeId=${list.typeId}" class="item_title">${list.itemTitle }</a>
