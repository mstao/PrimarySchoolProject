						...
						success:function(data){
							...
							if(data==1){
								layer.msg('删除成功', {icon: 1,time:2000});
									...
									window.location.href="${pageContext.request.contextPath}/admin/list/${durl}?classId=${classId}&menuId=${menuId}&p=1";
  		...
  		$.ajax({
  			...
  			success:function(data){
  				...
  				$.each(data,function(id,item){
  						if(item.isPublish==0){
  							xhtml+="<tr><td width='5%' align='center'><input type='checkbox' name='info_id' value="+item.id+"></td><td width='35%'><a href='${CTP}/admin/distribute/${durl }?id="+item.id+"&labId="+item.labId+"&labName=${labName}&menuId="+item.menuId+"' class='item_title'>"+item.itemTitle+"</a><span class='draft-span'>草稿</span></td><td width='20%'>"+item.author+"</td><td width='20%'>${menuName}</td><td width='20%'>"+item.addTime+"</td></tr>";
  							}
  						else{
  							xhtml+="<tr><td width='5%' align='center'><input type='checkbox' name='info_id' value="+item.id+"></td><td width='35%'><a href='${CTP}/admin/distribute/${durl }?id="+item.id+"&&labId="+item.labId+"&labName=${labName}&menuId="+item.menuId+"' class='item_title'>"+item.itemTitle+"</a></td><td width='20%'>"+item.author+"</td><td width='20%'>${menuName}</td><td width='20%'>"+item.addTime+"</td></tr>";
  						}
...
<jsp:include page="../../common/browsehappy.jsp"></jsp:include>
	...
	<!--S main-->
					<div id="location">您现在的位置: 后台管理 ><span>${classId }年级</span></div>
        		    <div class="new_div1"><span class="new_div1_span">${menuName }列表</span><a href="${CTP }/admin/write/${durl }?classId=${classId }&menuId=${menuId }&fileType=${fileType}" class="add_list">新增</a></div>
            ...
            <table>
                ...
                <tr>
                    ...
                    <td width="35%">
                    		 ...
                    		 <a href="${CTP}/admin/distribute/${durl }?id=${list.id}&classId=${classId}&menuId=${menuId}&typeId=${list.typeId}" class="item_title">${list.itemTitle }</a>
