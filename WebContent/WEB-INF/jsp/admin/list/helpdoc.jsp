...
$(function() {
   ...
   $(".rest-selectss").bind("click",function(){
		
		$(".mark").show();
		$(".dialog").show();
		
		//获取id的值
		sid=$(".rest-selectss").attr('data-id');
		//$("input[type='radio']:checked").val();
	});
	...
	//	buttonImage		: 'img/.....png',	// 按钮的背景图片,会覆盖文字
       ...
       onQueueComplete : function(queueData) {
               ...
               if(queueData.uploadsErrored==0){
               	...
               	//此时可以进行跳转页面
               	//跳转界面
					window.location.href=CTPPATH+"/admin/list/helpdoc";
               }else{
...
<div id="location"><span>您现在的位置: 后台管理 ></span><span>帮助文档</span></div>
<!-- 作息时间表 -->
  <div class="helps">
  		<div class="line">
	  		...
	  		<c:choose>
	  			...
	  			<c:otherwise>
	  				...
	  				<a href="javascript:location.href='${CTP}/download/file?fileName='+encodeURIComponent('${line.fileName}')+'&realName='+encodeURIComponent('${line.realName}');">${line.fileName}</a><span>${line.addTime}</span></li>
	  				</c:forEach>
	  				</ol>
	  				</div>
	  			</c:otherwise>
  		...
  		<div class="document">
	  		...
	  		<c:choose>
	  			...
	  			<c:otherwise>
	  				...
	  				<li><img src="${CTP_HOME}/img/j_do.png" class="dot"/>
	  				<a href="javascript:location.href='${CTP}/download/file?fileName='+encodeURIComponent('${document.fileName}')+'&realName='+encodeURIComponent('${document.realName}');">${document.fileName}</a><span>${document.addTime}</span></li>
	  				</c:forEach>
	  				</ol>
	  				</div>
	  			</c:otherwise>
  		...
  		<div class="count">
	  		...
	  		<c:choose>
	  			<c:when test="${empty count}">
	  				<a href="#" class="rest-selectss" data-id="3">新增</a>
	  			</c:when>
	  			<c:otherwise>
	  				<a href="#" class="rest-selectss" data-id="3">更新</a>
	  				<div class="rest-content">
	  				<ol>
	  				<c:forEach var="document" items="${count}">  
	  				<li><img src="${CTP_HOME}/img/j_do.png" class="dot"/>
	  				<a href="javascript:location.href='${CTP}/download/file?fileName='+encodeURIComponent('${count.fileName}')+'&realName='+encodeURIComponent('${count.realName}');">${count.fileName}</a><span>${count.addTime}</span></li>
	  				</c:forEach>
	  				</ol>
	  				</div>
	  			</c:otherwise>
