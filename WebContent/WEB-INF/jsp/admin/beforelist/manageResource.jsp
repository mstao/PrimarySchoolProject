...
<div id="location"><span>您现在的位置: 后台管理 ></span><span>${name}管理</span></div>
    ...
    <div class="teachingresource">
		<c:set var="url" value="manageResource"></c:set>
		<c:choose>
			<c:when test="${durl == url}">
				<c:forEach items="${menu}" var="menu_list">
				<span><input type="checkbox" name="info_id" value="${menu_list.id}"/><a href="${CTP}/admin/list/resource?classId=1&menuId=${menu_list.id}&p=1">${menu_list.resourceName}</a></span>
				</c:forEach>
			</c:when>
			<c:otherwise>
				<c:forEach items="${menu}" var="menu_list">
				<span><input type="checkbox" name="info_id" value="${menu_list.id}"/><a href="${CTP}/admin/list/department?deptId=${menu_list.id }&p=1">${menu_list.departmentName}</a></span>
				</c:forEach>
			</c:otherwise>
		</c:choose>
