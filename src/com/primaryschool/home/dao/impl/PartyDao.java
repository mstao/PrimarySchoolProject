...
public class PartyDao<T> implements IPartyDao<T> {
	...
	public T findPartyInfoById(int id) {
		// TODO Auto-generated method stub
		String hql="select new com.primaryschool.home.entity.Party(t.id,t.itemTitle,t.itemContent,t.addTime,t.viewCount,tt.itemTypeName,tt.itemTypeFlag)from Party t,PartyType tt  where tt.id=t.typeId and t.id=? and t.isPublish=1";
		Query query=sessionFactory.getCurrentSession().createQuery(hql);
		query.setInteger(0, id);
	...
	public List<T> findSliderPartyInfo(String flag, int position, int item_per_page) {
		...
		String hql="select new com.primaryschool.home.entity.Party(t.id,t.itemTitle,t.itemContent,t.addTime,tt.itemTypeFlag) from Party t,PartyType tt where t.typeId=? and t.typeId=tt.id and t.isPublish=1 and t.isImage=1 order by t.addTime desc";
		Query query=sessionFactory.getCurrentSession().createQuery(hql);
		query.setInteger(0, id);
		query.setFirstResult(position);
		query.setMaxResults(item_per_page);
