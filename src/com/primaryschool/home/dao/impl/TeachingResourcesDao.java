...
public class TeachingResourcesDao<T> implements ITeachingResourcesDao<T> {
	...
	public List<T> findSliderTeachingResourcesContent(int menuId, int classId, int position,
		...
		String hql="select new com.primaryschool.home.entity.TeachingResourcesContent(t.id,t.itemTitle,tct.itemTypeFlag,t.imagePath) from TeachingResourcesContent t,TeachingResourcesContentType  tct  where t.menuId=? and t.classId=?  and t.typeId=tct.id and t.isPublish=1 and t.isImage=1 order by t.addTime desc";
		Query query=sessionFactory.getCurrentSession().createQuery(hql);
		query.setInteger(0, menuId);
		query.setInteger(1, classId);
		query.setFirstResult(position);
