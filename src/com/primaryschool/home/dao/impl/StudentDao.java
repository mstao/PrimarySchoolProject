...
public class StudentDao<T> implements IStudentDao<T> {
	...
	public List<T> findSliderStudentInfo(String flag, int position, int item_per_page) {
		...
		String hql="select new com.primaryschool.home.entity.Student(t.id,t.itemTitle,t.itemContent,t.addTime) from Student t where t.typeId=? and t.isPublish=1 and t.isImage=1 order by t.addTime desc";
		Query query=sessionFactory.getCurrentSession().createQuery(hql);
		query.setInteger(0, id);
		query.setFirstResult(position);
		query.setMaxResults(item_per_page);
