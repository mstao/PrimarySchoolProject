...
public class ManageDao<T> implements IManageDao<T> {
	...
	public List<T> findSliderManageInfo(String flag, int position, int item_per_page) {
		...
		String hql="select new com.primaryschool.home.entity.Manage(t.id,t.itemTitle,tt.itemTypeFlag,t.imagePath) from Manage t,ManageType tt  where t.typeId=? and t.typeId=tt.id and t.isPublish=1 and t.isImage=1 order by t.addTime desc";
		Query query=sessionFactory.getCurrentSession().createQuery(hql);
		query.setInteger(0, id);
		query.setFirstResult(position);
		query.setMaxResults(item_per_page);
