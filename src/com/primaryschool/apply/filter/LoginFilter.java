package com.primaryschool.apply.filter;
...
public class LoginFilter implements Filter {  
    ...
    private static final String LOGIN_CARD_CODE = "LOGIN_CARD_CODE";      //登录用户在session中的属性key -- session.setAttribute(key,value)  
    ...
    public void init(FilterConfig config) throws ServletException {  
        ...
        loginCardCode = config.getInitParameter(LOGIN_CARD_CODE);  
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {  
         ...
         if(servletPath.equals(showloginPath) ){  
           ...
           String cardCode= (String) httpReq.getSession().getAttribute(loginCardCode);  
           //System.out.print(username);
           if(cardCode != null){//如果不为空,则进行已登录处理  
                 chain.doFilter(request, response);  
